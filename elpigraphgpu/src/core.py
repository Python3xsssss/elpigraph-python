...
def PartitionData_and_get_Hamerley_bounds(Xcp, NodePositions, MaxBlockSize, SquaredXcp,
    ...
    idx_all = cp.arange(n)
    partition = cp.zeros((n, 1), dtype=int)
    partition2nd = cp.zeros((n, 1), dtype=int)
    partition3rd = cp.zeros((n, 1), dtype=int)
    ...
    for i in range(0, n, MaxBlockSize):
        ...
        partition[i:last] = tmp[:, cp.newaxis]
        partition2nd[i:last] = tmp2[:, cp.newaxis]
        partition3rd[i:last] = tmp3[:, cp.newaxis]
        dists[i:last] = d[cp.arange(d.shape[0]), tmp][:, cp.newaxis]
...
def PrimitiveElasticGraphEmbedment_feed_partition_dists(X, NodePositions, ElasticMatrix, partition, dists,
                                   MaxNumberOfIterations=10, eps=0.01,
                                   Mode = 1, FinalEnergy = "Base",
                                   alpha = 0,
                                   beta = 0,
...
#                                              SquaredX, TrimmingRadius)
    ...
    for i in range(MaxNumberOfIterations):
        ...
        if diff < eps:
            ...
            #nbrs.fit(NewNodePositions)
            #dists, partition = nbrs.kneighbors(X)
